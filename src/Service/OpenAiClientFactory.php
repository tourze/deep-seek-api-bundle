...
class OpenAiClientFactory
{
    ...
    /**
     * @param array<string, mixed> $config
     * @deprecated Use createClient() with DeepSeekApiKey entity instead
     */
    public function createClientFromApiKeyString(string $apiKeyString, array $config = []): AbstractOpenAiClient
    {
        $baseUri = is_string($config['base_uri'] ?? null) ? $config['base_uri'] : 'https://api.deepseek.com';
        $timeout = is_int($config['timeout'] ?? null) ? $config['timeout'] : 30;
        ...
        $client = new DeepSeekOpenAiClient($httpClient, $maxRetries);
